...
Cloud model workers remain in the private `modal-3D` repository. Local SAM 3.1 will be optional and is not installed by the base client. Modal credentials are validated through the localhost Agent. On Windows they can be stored in Windows Credential Manager and restored directly into the Agent without loading the saved secret back into React.
